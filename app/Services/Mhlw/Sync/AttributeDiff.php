 ...
 * attribute array (from app/Services/Mhlw/Import/*RowMapper) and reports
 * only the keys that actually changed. Pure logic, no DB access.
 *
 * Values are normalized before comparison rather than using a bare `===`:
 * MedicalFacility::latitude/longitude are cast `decimal:6`, which Laravel
 * always returns as a *string* (e.g. "35.658581"), while every row mapper
 * produces a PHP float for the same column -- an unnormalized `===` would
 * flag every facility with coordinates as "changed" on every single
 * reimport. JSON-valued columns are also key-sorted before comparing so a
 * future mapper refactor that builds an array in a different (but
 * content-equal) key order can't silently trigger the same kind of
 * false-positive Updated event flood.
 */
