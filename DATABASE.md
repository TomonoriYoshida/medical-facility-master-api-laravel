...
- 実CSVインポート機能は複数フェーズに分けて実装中。フェーズ1（スキーマ修正・廃業検知用監視カラムの追加）・フェーズ2（CSV→構造化データのパース層、`app/Services/Mhlw/Import/`配下）・フェーズ3（差分検出・upsert層、`app/Services/Mhlw/Sync/`配下）は完了。フェーズ4（Job・Queue・`mhlw:import`コマンド）は今後対応する。定期クロール・REST API・認証も別フェーズで対応する。
